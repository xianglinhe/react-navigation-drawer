...
export default class DrawerLayout extends Component<PropType, StateType> {
  ...
  _tapGestureHandler = ({nativeEvent}) => {
    //when touch is ending ,close the drawView
    if (nativeEvent.state === 5 && this.state.drawerShown) {  //5: release the touch
    this.closeDrawer()
    }
  }
  ...
  render() {
    ...
    let enableDrawer = this.props.drawerLockMode === 'unlocked'
    return <PanGestureHandler enabled={enableDrawer} hitSlop={this.props.stackCount == 1 ? 0 : { left: -this.props.hitSlopWidth, right: 0 }} activeOffsetX={offset} onGestureEvent={this._onGestureEvent} onHandlerStateChange={this._openingHandlerStateChange} ref={ref => this.panGesture = ref}>
          {this._renderDrawer()}
      </PanGestureHandler>
  }
}
